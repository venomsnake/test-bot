...
from Nibb import (API_HASH, API_ID, BOT_TOKEN, LOG_DATETIME, LOGFILE, LOGGER,
                    MESSAGE_DUMP, NO_LOAD, OWNER_ID, UPTIME, WORKERS,
                    load_cmds)
from Nibb.database import MongoDB
from Nibb.plugins import all_plugins
...
class Ceregay(Client):
    """Starts the Pyrogram Client on the Bot Token when we do 'python3 -m Nibb'"""
    ...
    def __init__(self):
        ...
        super().__init__(
            "Nibbs",
            bot_token=BOT_TOKEN,
            plugins=dict(root="Nibb.plugins", exclude=NO_LOAD),
            api_id=API_ID,
            api_hash=API_HASH,
            workers=WORKERS,
        )
