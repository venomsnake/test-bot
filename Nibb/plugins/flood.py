...
from Nibb import LOGGER, SUPPORT_GROUP, SUPPORT_STAFF
from Nibb.bot_class import Ceregay
from Nibb.database.approve_db import Approve
from Nibb.database.flood_db import Floods
from Nibb.utils.custom_filters import admin_filter, command
from Nibb.utils.extras import BAN_GIFS, KICK_GIFS, MUTE_GIFS
...
@Ceregay.on_message(command(['floodaction','actionflood']) & admin_filter)
async def flood_action(c: Ceregay, m: Message):
    Flood = Floods()
...
@Ceregay.on_message(command(['isflood', 'flood']) & ~filters.bot)
async def flood_on_off(c: Ceregay, m: Message):
    if m.chat.type == CT.PRIVATE:   
...
@Ceregay.on_message(command(['setflood']) & ~filters.bot & admin_filter)
async def flood_set(c: Ceregay, m: Message):
    bot = await c.get_chat_member(m.chat.id, Config.BOT_ID)
    Flood = Floods()
    status = bot.status
    if not status in [CMS.OWNER, CMS.ADMINISTRATOR]:
...
@Ceregay.on_callback_query(filters.regex("^f_"))
async def callbacks(c: Ceregay, q: CallbackQuery):
    data = q.data
    if data == "f_close":
...
@Ceregay.on_callback_query(filters.regex("^un_"))
async def reverse_callbacks(c: Ceregay, q: CallbackQuery):
    data = q.data.split("_")
    action = data[1]
...
dic = {}
@Ceregay.on_message(filters.all & ~filters.bot | ~filters.private, 10)
async def flood_watcher(c: Ceregay, m: Message):
    c_id = m.chat.id
    Flood = Floods()
    try:
        u_id = m.from_user.id
