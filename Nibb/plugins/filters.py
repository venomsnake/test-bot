...
from Nibb.bot_class import LOGGER, Ceregay
...
from Nibb.utils.msg_types import Types, get_filter_type
from Nibb.utils.regex_utils import regex_searcher
...
@Ceregay.on_message(command("filters") & filters.group & ~filters.bot)
...
@Ceregay.on_message(command(["filter", "addfilter"]) & admin_filter & ~filters.bot)
...
@Ceregay.on_message(command(["stop", "unfilter"]) & admin_filter & ~filters.bot)
...
@Ceregay.on_callback_query(filters.regex("^rm_allfilters$"))
...
async def send_filter_reply(c: Ceregay, m: Message, trigger: str):
...
@Ceregay.on_message(filters.text & filters.group & ~filters.bot, group=69)
async def filters_watcher(c: Ceregay, m: Message):
