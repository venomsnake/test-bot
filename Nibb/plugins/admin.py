...
from Nibb import DEV_USERS, LOGGER, OWNER_ID, SUPPORT_GROUP, SUPPORT_STAFF
...
from Nibb.utils.caching import (ADMIN_CACHE, TEMP_ADMIN_CACHE_BLOCK,
                                  admin_cache_reload)
from Nibb.utils.custom_filters import (DEV_LEVEL, admin_filter, command,
                                         owner_filter, promote_filter)
from Nibb.utils.extract_user import extract_user
from Nibb.utils.parser import mention_html
from Nibb.vars import Config


@Ceregay.on_message(command("adminlist"))
...
@Ceregay.on_message(command("zombies") & owner_filter)
async def zombie_clean(c: Ceregay, m: Message):
    zombie = 0
...
@Ceregay.on_message(command("admincache"))
...
@Ceregay.on_message(filters.regex(r"^(?i)@admin(s)?") & filters.group)
...
@Ceregay.on_message(command("fullpromote") & promote_filter)
async def fullpromote_usr(c: Ceregay, m: Message):
    global ADMIN_CACHE
    ...
    try:
        ...
        if m.chat.type in [ChatType.SUPERGROUP, ChatType.GROUP]:
            title = "Ceregay"  # Default fullpromote title
...
@Ceregay.on_message(command("promote") & promote_filter)
async def promote_usr(c: Ceregay, m: Message):
    global ADMIN_CACHE
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(
...
@Ceregay.on_message(command("demote") & promote_filter)
async def demote_usr(c: Ceregay, m: Message):
    global ADMIN_CACHE
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@Ceregay.on_message(command("invitelink"))
async def get_invitelink(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("setgtitle") & admin_filter)
...
@Ceregay.on_message(command("setgdes") & admin_filter)
...
@Ceregay.on_message(command("title") & admin_filter)
async def set_user_title(c: Ceregay, m: Message):
    user = await m.chat.get_member(m.from_user.id)
...
@Ceregay.on_message(command("setgpic") & admin_filter)
async def setgpic(c: Ceregay, m: Message):
    user = await m.chat.get_member(m.from_user.id)
    if not user.privileges.can_change_info and user.status != CMS.OWNER:
        await m.reply_text(
            "You don't have enough permission to use this command!",
