...
async def warn(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("resetwarns") & restrict_filter)
async def reset_warn(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("warns") & filters.group)
async def list_warns(c: Ceregay, m: Message):
...
async def remove_warn(c: Ceregay, m: Message):
...
@Ceregay.on_callback_query(filters.regex("^warn."))
async def remove_last_warn_btn(c: Ceregay, q: CallbackQuery):
...
@Ceregay.on_message(command(["warnings", "warnsettings"]) & admin_filter)
...
@Ceregay.on_message(command("warnmode") & admin_filter)
...
@Ceregay.on_message(command("warnlimit") & admin_filter)
