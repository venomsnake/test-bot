...
from Nibb import (BOT_TOKEN, DEV_USERS, LOG_DATETIME, LOGFILE, LOGGER,
                    MESSAGE_DUMP, OWNER_ID, UPTIME)
from Nibb.bot_class import Ceregay
from Nibb.database import MongoDB
from Nibb.database.chats_db import Chats
from Nibb.utils.clean_file import remove_markdown_and_html
from Nibb.utils.custom_filters import command
from Nibb.utils.extract_user import extract_user
from Nibb.utils.http_helper import *
from Nibb.utils.parser import mention_markdown


@Ceregay.on_message(command(["adddev", "rmdev"]))
async def add_dev(c: Ceregay, m:Message):
...
@Ceregay.on_message(command("ping", sudo_cmd=True))
...
@Ceregay.on_message(command("logs", dev_cmd=True))
async def send_log(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("neofetch", dev_cmd=True))
...
@Ceregay.on_message(command(["eval", "py"], dev_cmd=True))
async def evaluate_code(c: Ceregay, m: Message):
...
@Ceregay.on_message(command(["exec", "sh"], dev_cmd=True))
async def execution(c: Ceregay, m: Message):
    protect = BOT_TOKEN.split(":")
    initial = protect[0]
    end = protect[1][-5:]
    if len(m.text.split()) == 1:
...
async def stop_and_send_logger(c:Ceregay,is_update=False):
...
@Ceregay.on_message(command(["restart", "update"], owner_cmd=True))
async def restart_the_bot(c:Ceregay,m:Message):
    try:
        cmds = m.command
        ...
        execvp(executable, [executable, "-m", "Nibb"])
    except Exception as e:
...
@Ceregay.on_message(command("chatlist", dev_cmd=True))
async def chats(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("uptime", dev_cmd=True))
...
@Ceregay.on_message(command("leavechat", dev_cmd=True))
async def leave_chat(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("chatbroadcast", dev_cmd=True))
async def chat_broadcast(c: Ceregay, m: Message):
    if m.reply_to_message:
