...
from Nibb import LOGGER, MESSAGE_DUMP, SUPPORT_STAFF
from Nibb.bot_class import Ceregay
from Nibb.database.antispam_db import ANTISPAM_BANNED, GBan
from Nibb.database.approve_db import Approve
from Nibb.database.blacklist_db import Blacklist
from Nibb.database.group_blacklist import BLACKLIST_CHATS
...
@Ceregay.on_message(filters.linked_channel)
async def antichanpin_cleanlinked(c: Ceregay, m: Message):
    try:
        msg_id = m.id
...
@Ceregay.on_message(filters.text & filters.group, group=5)
...
@Ceregay.on_message(filters.user(list(ANTISPAM_BANNED)) & filters.group)
async def gban_watcher(c: Ceregay, m: Message):
    from Nibb import SUPPORT_GROUP
...
@Ceregay.on_message(filters.chat(BLACKLIST_CHATS))
async def bl_chats_watcher(c: Ceregay, m: Message):
    from Nibb import SUPPORT_GROUP
