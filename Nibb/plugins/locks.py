...
from Nibb import DEV_USERS, LOGGER, OWNER_ID, SUDO_USERS
...
anti_forward_u = []
anti_forward_c = []
@Ceregay.on_message(command("locktypes"))
...
@Ceregay.on_message(command("lock") & restrict_filter)
async def lock_perm(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("locks") & restrict_filter)
...
@Ceregay.on_message(command("unlock") & restrict_filter)
async def unlock_perm(c: Ceregay, m: Message):
    if len(m.text.split()) < 2:
...
async def delete_messages(c:Ceregay, m: Message):
    try:
        await m.delete()
        return
...
async def is_approved_user(c:Ceregay, m: Message):
...
@Ceregay.on_message(filters.all & ~filters.me,18)
async def lock_del_mess(c:Ceregay, m: Message):
