...
from Nibb import DEV_USERS
from Nibb.bot_class import Ceregay
from Nibb.database.antispam_db import GBan
from Nibb.database.greetings_db import Greetings
from Nibb.utils.custom_filters import admin_filter, bot_admin_filter, command
from Nibb.utils.msg_types import Types, get_wlcm_type
from Nibb.utils.parser import escape_markdown, mention_html
from Nibb.utils.string import (build_keyboard, escape_invalid_curly_brackets,
                                 parse_button)
...
@Ceregay.on_message(command("cleanwelcome") & admin_filter)
...
@Ceregay.on_message(command("cleangoodbye") & admin_filter)
...
@Ceregay.on_message(command("cleanservice") & admin_filter)
...
@Ceregay.on_message(command("setwelcome") & admin_filter & bot_admin_filter)
...
@Ceregay.on_message(command("setgoodbye") & admin_filter & bot_admin_filter)
...
@Ceregay.on_message(command("resetgoodbye") & admin_filter & bot_admin_filter)
...
@Ceregay.on_message(command("resetwelcome") & admin_filter & bot_admin_filter)
...
@Ceregay.on_message(filters.service & filters.group, group=59)
...
@Ceregay.on_chat_member_updated(filters.group, group=69)
async def member_has_joined(c: Ceregay, member: ChatMemberUpdated):
...
@Ceregay.on_chat_member_updated(filters.group, group=99)
async def member_has_left(c: Ceregay, member: ChatMemberUpdated):
...
@Ceregay.on_message(command("welcome") & admin_filter)
async def welcome(c: Ceregay, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_welcome_status()
    oo = db.get_welcome_text()
    args = m.text.split(" ", 1)
...
@Ceregay.on_message(command("goodbye") & admin_filter)
async def goodbye(c: Ceregay, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_goodbye_status()
    oo = db.get_goodbye_text()
    args = m.text.split(" ", 1)
...
× Ceregay must be an admin to greet and goodbye users.
