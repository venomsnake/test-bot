...
from Nibb.database.giveaway_db import GIVEAWAY
...
@Ceregay.on_message(command(["startgiveaway", "startga"]))
async def start_give_one(c: Ceregay, m: Message):
    uWu = True
    try:
...
async def message_editor(c:Ceregay, m: Message, c_id):
    txt = f"""
**#Giveaway 》**
➖➖➖➖➖➖➖➖➖➖➖
__To win this logo giveaway__
...
@Ceregay.on_message(command("stopentry"))
async def stop_give_entry(c:Ceregay, m: Message):
    GA = GIVEAWAY()
    u_id = m.from_user.id
...
@Ceregay.on_message(command(["stopgiveaway","stopga"]))
async def stop_give_away(c:Ceregay, m: Message):
    GA = GIVEAWAY()
    u_id = m.from_user.id
    curr = GA.give_info(u_id=u_id)
    if not curr:
...
@Ceregay.on_message(command("startvote"))
async def start_the_vote(c: Ceregay, m: Message):
    GA = GIVEAWAY()
    u_id = m.from_user.id
    curr = GA.give_info(u_id=m.from_user.id)
    if not curr:
...
@Ceregay.on_message(command(["enter","register","participate"]))
async def register_user(c: Ceregay, m: Message):
    GA = GIVEAWAY()
...
@Ceregay.on_callback_query(filters.regex("^vote_"))
async def vote_increment(c: Ceregay, q: CallbackQuery):
    GA = GIVEAWAY()
    data = q.data.split("_")
    c_id = int(data[1])
    u_id = int(data[2])
...
@Ceregay.on_message(filters.left_chat_member)
async def rejoin_try_not(c:Ceregay, m: Message):
