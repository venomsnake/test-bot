...
from Nibb import LOGGER, MESSAGE_DUMP, SUPPORT_GROUP, SUPPORT_STAFF
...
@Ceregay.on_message(command(["gban", "globalban"], sudo_cmd=True))
async def gban(c: Ceregay, m: Message):
...
async def ungban(c: Ceregay, m: Message):
    if len(m.text.split()) == 1:
