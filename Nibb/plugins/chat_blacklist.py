...
from Nibb.database.group_blacklist import GroupBlacklist
...
@Ceregay.on_message(command("blchat", dev_cmd=True))
async def blacklist_chat(c: Ceregay, m: Message):
...
async def unblacklist_chat(c: Ceregay, m: Message):
    if len(m.text.split()) >= 2:
        chat_ids = m.text.split()[1:]
