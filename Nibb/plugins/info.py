...
from Nibb import (DEV_USERS, LOGGER, OWNER_ID, SUDO_USERS, SUPPORT_STAFF,
                    WHITELIST_USERS)
...
async def count(c: Ceregay, chat):
    try:
        administrator = []
...
async def user_info(c: Ceregay, user, already=False):
    if not already:
...
async def chat_info(c: Ceregay, chat, already=False):
    if not already:
        try:
...
@Ceregay.on_message(command(["info", "whois"]))
async def info_func(c: Ceregay, message: Message):
...
@Ceregay.on_message(command(["chinfo", "chatinfo", "chat_info"]))
async def chat_info_func(c: Ceregay, message: Message):
