...
from Nibb import LOGGER, OWNER_ID, SUPPORT_GROUP, SUPPORT_STAFF
from Nibb.bot_class import Ceregay
from Nibb.utils.caching import ADMIN_CACHE, admin_cache_reload
from Nibb.utils.custom_filters import command, restrict_filter
from Nibb.utils.extract_user import extract_user
from Nibb.utils.extras import MUTE_GIFS
from Nibb.utils.parser import mention_html
from Nibb.utils.string import extract_time
from Nibb.vars import Config


@Ceregay.on_message(command("tmute") & restrict_filter)
async def tmute_usr(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("dtmute") & restrict_filter)
async def dtmute_usr(c: Ceregay, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text("I can't mute nothing!")
        return
...
@Ceregay.on_message(command("stmute") & restrict_filter)
async def stmute_usr(c: Ceregay, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text("I can't mute nothing!")
        return
...
@Ceregay.on_message(command("mute") & restrict_filter)
async def mute_usr(c: Ceregay, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text("I can't mute nothing!")
        return
...
@Ceregay.on_message(command("smute") & restrict_filter)
async def smute_usr(c: Ceregay, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text("I can't mute nothing!")
        return
...
@Ceregay.on_message(command("dmute") & restrict_filter)
async def dmute_usr(c: Ceregay, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text("I can't mute nothing!")
        return
    if not m.reply_to_message:
...
@Ceregay.on_message(command("unmute") & restrict_filter)
async def unmute_usr(c: Ceregay, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@Ceregay.on_callback_query(regex("^unmute_"))
async def unmutebutton(c: Ceregay, q: CallbackQuery):
