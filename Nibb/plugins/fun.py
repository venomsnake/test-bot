...
from Nibb import DEV_USERS, LOGGER
from Nibb.bot_class import Ceregay
from Nibb.utils import extras
from Nibb.utils.custom_filters import command
from Nibb.utils.extras import NOWYES as NO
from Nibb.utils.extras import YESWNO as YES


@Ceregay.on_message(command("shout"))
...
@Ceregay.on_message(command("runs"))
...
@Ceregay.on_message(command("slap"))
async def fun_slap(c: Ceregay, m: Message):
    me = await c.get_me()
    ...
    if user_id == me.id:
        temp = choice(extras.SLAP_Ceregay_TEMPLATES)
    else:
...
@Ceregay.on_message(command("roll"))
...
@Ceregay.on_message(command("toss"))
...
@Ceregay.on_message(command("insult"))
async def insult(c: Ceregay, m: Message):
...
@Ceregay.on_message(command("yes"))
async def yesw(c: Ceregay, m: Message):
    reply_text = m.reply_to_message.reply_text if m.reply_to_message else m.reply_text
    rtext = YES[0]
...
@Ceregay.on_message(command("no"))
async def now(c: Ceregay, m: Message):
    reply_text = m.reply_to_message.reply_text if m.reply_to_message else m.reply_text
    rtext = NO[0]
    await reply_text(rtext)
...
@Ceregay.on_message(command("shrug"))
...
@Ceregay.on_message(command("bluetext"))
...
@Ceregay.on_message(command("decide"))
...
@Ceregay.on_message(command("react"))
...
@Ceregay.on_message(command("weebify"))
