...
from Nibb import LOGGER, SUPPORT_CHANNEL
...
@Ceregay.on_message(command('google'))
async def g_search(c: Ceregay, m: Message):
...
@Ceregay.on_message(command('anime'))
async def anime_search(c: Ceregay, m: Message):
    split = m.text.split(None, 1)
    if len(split) == 1:
        return await m.reply_text("No query given\nDo `/help search` to see how to use it")
...
@Ceregay.on_message(command('stack'))
async def stack_search(c: Ceregay, m: Message):
    split = m.text.split(None, 1)
    if len(split) == 1:
        return await m.reply_text("No query given\nDo `/help search` to see how to use it")
